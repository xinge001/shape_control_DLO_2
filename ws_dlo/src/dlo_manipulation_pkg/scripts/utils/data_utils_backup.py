...
def get_graph_data(fp_pos, fp_pos_d, end_pos, end_pos_d, delta_t, miss_idx=[], shift=True,
                            normalize_v=True, use_yaw_d = False):
    ...
    if normalize_v:
        ...
        end_pos_d[:, 2] = end_pos_d[:, 2] / 20  # scale yaw, otherwise too large
        if fp_pos_d is not None:
...
class GraphDataset(pyg.data.Dataset):
    ...
    def get(self, idx):
        ...
        graph = get_graph_data(fp_pos, fp_pos_d, end_pos, end_pos_d, delta_t, use_yaw_d=self.args.use_yaw_d)
