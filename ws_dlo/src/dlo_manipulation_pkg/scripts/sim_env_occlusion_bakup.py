...
from trial import correct_shape, check_distance_anomalies
...
def check_lr_small(mysimulator):
    lr_small = False
    for i, subnet in enumerate(mysimulator.subnets):
        if i in args.train_nets and subnet.lr<1e-6:
            print("lr_small", lr_small)
            return True
    return lr_small
